...
def grow_polymer(old_poly):
    global rules
    new_poly = []
    for p in range(len(old_poly) - 1):
        new_poly.append(old_poly[p])
        new_poly.append(rules[f"{old_poly[p]}{old_poly[p+1]}"])
    new_poly.append(old_poly[-1])
    return new_poly


def count_parts(poly):
    poly_dict = {}
    for part in poly:
        try:
            poly_dict[part] += 1
...
for step in range(10):
    ...
    print(f"{step + 1}: {len(polymer)}")

parts_dict = count_parts(polymer)
